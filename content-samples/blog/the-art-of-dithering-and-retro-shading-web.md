...
- [Dithering on the GPU](http://alex-charlton.com/Dithering_on_the_GPU/) from Alex Charlton which features a nice deep dive into alternative color quantization techniques
...
For this project, we'll create a **custom post-processing effect**. As we did for the [Moebius stylized shader](/moebius-style-post-processing/), relying on post-processing will allow us to apply a shader to an already rendered scene and alter its style like adding an "image filter" to a photo.
...
One of the first tweaks I felt was necessary was to add some slight chromatic aberration when sampling the input buffer of our effect, which we can find the code of in [Refraction, dispersion, and other shader light effects](/refraction-dispersion-and-other-shader-light-effects/). Due to the screen curvature and some imperfect alignments of the electron beams of the CRT, it was frequent that color channels would appear slightly offset, yielding a slightly blurred image.
