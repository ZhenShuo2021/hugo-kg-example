...
$ curl --unix-socket /var/run/docker.sock -H "Content-Type: application/json" \
  ...
  -X POST http://localhost/v/containers/create
{"Id":"1c6594faf5","Warnings":null}

$ curl --unix-socket /var/run/docker.sock -X POST http://localhost/v/containers/1c6594faf5/start

$ curl --unix-socket /var/run/docker.sock -X POST http://localhost/v/containers/1c6594faf5/wait
{"StatusCode":0}

$ curl --unix-socket /var/run/docker.sock "http://localhost/v/containers/1c6594faf5/logs?stdout=1"
hello world
```
...
> for example: `http:/v/containers/1c6594faf5/start`.
...
```console
$ curl --unix-socket /var/run/docker.sock http://localhost/v/containers/json
[{
...
$ curl --unix-socket /var/run/docker.sock \
  -X POST http://localhost/v/containers/ae63e8b89a26/stop
```
...
```console
$ curl --unix-socket /var/run/docker.sock "http://localhost/v/containers/ca5f55cdb/logs?stdout=1"
...
```console
$ curl --unix-socket /var/run/docker.sock http://localhost/v/images/json
[{
...
```console
$ curl --unix-socket /var/run/docker.sock \
  -X POST "http://localhost/v/images/create?fromImage=alpine"
...
$ curl --unix-socket /var/run/docker.sock\
  -X POST "http://localhost/v/commit?container=0888269a9d&repo=helloworld"
